...
export default async function Image({ params }: { params: Promise<{ id: string }> }) {
  ...
  let report: {
    message: string; status: string; created_at: Date; user_name: string;
    assignee_name: string | null; deadline: Date | null;
  } | null = null;

  try {
    const result = await pool.query(
      `SELECT dr.message, dr.status, dr.created_at, dr.deadline,
              u.name AS user_name, a.name AS assignee_name
          ...
          {(report.assignee_name || deadlineStr) && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginTop: 24 }}>
              ...
              {deadlineStr && (
                <div
                  style={{
                    display: 'flex', alignItems: 'center', background: '#ffedd5', color: '#c2410c',
                    fontSize: 26, fontWeight: 600, padding: '8px 20px', borderRadius: 12,
                  }}
                >
                  ⏰ {deadlineStr}
                </div>
